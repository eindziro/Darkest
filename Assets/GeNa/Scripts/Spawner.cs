...
namespace GeNa
{
    ...
    public class Spawner : MonoBehaviour
    {
		...
		public float m_terrainTopBoundary = 5000f;
		...
		private bool m_critHeightsInitedForTerrain = false;
        private float m_critMinHeight = 0f;
        private float m_critMaxHeight = 10000f;
		...
		public void SetSpawnOriginAndUpdateRanges(Transform groundObject, Vector3 location, Vector3 normal)
        {
            ...
            if (groundObject != null)
            {
				...
				if (terrainGround != null)
                {
					...
					if (!m_critHeightsInitedForTerrain)
					{
						m_showCritMinSpawnHeight = true;
						m_critMinSpawnHeight = 50f + groundObject.transform.position.y;
						m_terrainTopBoundary = groundObject.transform.position.y + terrainGround.terrainData.size.y;
						m_critMaxSpawnHeight = m_terrainTopBoundary;
						m_critHeightsInitedForTerrain = true;
					}					
                }
                else
                {
        ...
        public void UpdateTargetSpawnerRanges()
        {
			...
			if (m_critMinHeight > m_critMaxSpawnHeight)
			{
				m_critMinHeight = m_critMaxSpawnHeight;
			}
			...
			if (m_critMaxHeight > m_critMaxSpawnHeight)
			{
				m_critMaxHeight = m_critMaxSpawnHeight;
			}
...
#region Spawning methods
            ...
            for (; (instancesSpawned < instancesToSpawn) && (currLoop < maxLoops); currLoop++)
            {
                ...
                if (m_lastSpawnedObject != null && m_rotationAlgorithm > Constants.RotationAlgorithm.Fixed)
                {
                    if (m_rotationAlgorithm == Constants.RotationAlgorithm.LastSpawnClosest)
                    {
						...
						Vector3 closest = PWUtils.GetNearestVertice(newLocation, m_lastSpawnedObject);
        ...
        private bool PaintPrototype(Prototype prototype, Vector3 location, Vector3 normal, float alpha, Vector3 scaleFactor, float rotation, bool spawnAtLeastOneResource, out GameObject spawnedInstance)
        {
            ...
            foreach (Resource res in prototype.m_resources)
            {
                ...
                if (res.m_resourceType == Constants.ResourceType.Prefab)
                {
                    ...
                    //Set the local scale multiplier
                    Vector3 scaleMultiplier = Vector3.one;
                    if (res.m_sameScale)
                    {
                        float resScale = m_randomGenerator.Next(res.m_minScale.x, res.m_maxScale.x);
                        scaleMultiplier = new Vector3(resScale, resScale, resScale);
                    }
                    else
                    {
                        scaleMultiplier = new Vector3(m_randomGenerator.Next(res.m_minScale.x, res.m_maxScale.x),
                            m_randomGenerator.Next(res.m_minScale.y, res.m_maxScale.y),
                            m_randomGenerator.Next(res.m_minScale.z, res.m_maxScale.z));
                    }

                    if (m_scaleToNearestInt == true)
                    {
                        go.transform.localScale = ScaleToNearestInt(Vector3.Scale(scaleMultiplier, scaleFactor));
                    }
                    else
                    {
                        go.transform.localScale = Vector3.Scale(scaleMultiplier, scaleFactor);
                    }
                        ...
                        if (!string.IsNullOrEmpty(res.m_assetID))
                        {
                            ...
                            if (!string.IsNullOrEmpty(res.m_avsID))
                            {
                                VegetationStudioManager.AddVegetationItemInstance(res.m_avsID, go.transform.position,
                                    go.transform.localScale, go.transform.rotation, true, (byte)11, true);
                ...
                else if (res.m_resourceType == Constants.ResourceType.TerrainTree)
                {
                    if (terrain != null && res.m_terrainProtoIdx < terrain.terrainData.treePrototypes.Length)
                    {
                        ...
                        Vector3 newScale = Vector3.Scale(scaleMultiplier, scaleFactor);
                        if (m_scaleToNearestInt == true)
                        {
                            t.widthScale = Mathf.Ceil(Mathf.Max(newScale.x, newScale.z));
                            t.heightScale = Mathf.Ceil(newScale.y);
                        }
                        else
                        {
                            t.widthScale = Mathf.Max(newScale.x, newScale.z);
                            t.heightScale = newScale.y;
                        }
        ...
        private bool CheckBoundedLocationForSpawn(Vector3 location, float rotation, Prototype prototype, Vector3 extents, bool visualising)
        {
			...
			if (Physics.BoxCast(new Vector3(location.x, location.y + m_advSpawnCheckOffset, location.z), extents, 
				Vector3.down, out hitInfo, Quaternion.Euler(0f, rotation, 0f), m_advSpawnCheckOffset+1000f, m_critSpawnCollisionLayers))
            {
                //Do the height test
                if (m_critCheckHeight == true)
                {
...
#region Gizmos
        ...
        void OnDrawGizmosSelected()
        {
            ...
            if (m_critCheckHeight == true && (m_showCritMinSpawnHeight == true || m_showCritMaxSpawnHeight == true))
            {
