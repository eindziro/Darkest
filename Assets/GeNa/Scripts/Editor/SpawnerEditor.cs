...
namespace GeNa
{
	...
	public class SpawnerEditor : Editor
	{
	    ...
	    private class SpawnCall
	    {
	        ...
	        public SpawnCall(Transform transform, Vector3 location, Vector3 normal)
	        {
	            m_isSubspawn = false;
		...
		/// Show tutorial
		/// </summary>
		[MenuItem("GameObject/GeNa/Show GeNa Tutorials...", false, 40)]
		public static void ShowGeNaTutorial()
		{
			Application.OpenURL("http://www.procedural-worlds.com/gena/?section=tutorials");
		}

	    /// <summary>
	    /// Show discord
	    /// </summary>
	    [MenuItem("GameObject/GeNa/Show GeNa Discord, Have a Chat...", false, 41)]
	    public static void ShowGeNaDiscord()
	    {
	    ...
	    /// Show discord
	    /// </summary>
	    [MenuItem("GameObject/GeNa/Show GeNa Support, Lodge a Ticket...", false, 42)]
	    public static void ShowGeNaSupport()
	    {
	    ...
	    /// Show review option
	    /// </summary>
	    [MenuItem("GameObject/GeNa/Show Procedural Worlds Assets...", false, 43)]
	    ...
	    /// Show review option
	    /// </summary>
	    [MenuItem("GameObject/GeNa/Show Procedural Worlds Assets (SECTR)...", false, 44)]
	    public static void ShowAssetStoreSectr()
	    {
	        Application.OpenURL("https://www.assetstore.unity3d.com/en/?stay#!/search/page=1/sortby=popularity/query=publisher:6087");
	    }

        /// <summary>
        /// Show review option
        /// </summary>
        [MenuItem("GameObject/GeNa/Please Review GeNa...", false, 45)]
		public static void ShowGenaAssetStore()
		{
			Application.OpenURL("https://www.assetstore.unity3d.com/en/#!/content/74407?aid=1101lSqC");
		}
		...
		public override void OnInspectorGUI()
		{
		    ...
		    if (m_spawner.m_showHelp)
		    {
		        ...
		        if (GUI.Button(rect, "?-"))
		        {
		            m_spawner.m_showHelp = !m_spawner.m_showHelp;
		        }
			...
			if (m_spawner.m_showHelp)
			{
				...
				{
					GUILayout.Label(FIRST_STEP_HELP_LABEL);
					GUILayout.Label(VISUALISE_HELP_LABEL);
					GUILayout.Label(ROTATION_HELP_LABEL);
					GUILayout.Label(RANGE_HELP_LABEL);
					GUILayout.Label(INSTANCES_HELP_LABEL);
					GUILayout.Label(MOVE_LAST_HELP_LABEL);
					GUILayout.Label(HEIGHT_AND_ROT_LAST_HELP_LABEL);
					GUILayout.Label(DELETE_ALL_HELP_LABEL);
					GUILayout.Label(SINGLE_SPAWN_HELP_LABEL);
					GUILayout.Label(GLOBAL_SPAWN_HELP_LABEL);
				}
				else
				{
			...
			string parentName = m_spawner.m_parentName;
			Constants.SpawnMode spawnMode = m_spawner.m_spawnMode;
			float flowRate = Mathf.Sqrt(m_spawner.m_flowDistanceSqr);
			float flowRateChecksum = flowRate;
		    ...
		    parentName = EditorGUILayout.TextField(GetLabel("Name"), parentName);
            spawnMode = (Constants.SpawnMode)EditorGUILayout.EnumPopup(GetLabel("Spawn Mode"), spawnMode);
			...
			if (!advUseLargeRanges)
			{
				if (spawnMode > Constants.SpawnMode.Single)
				{
					...
					flowRate = EditorGUILayout.Slider(GetLabel("Flow Rate"), flowRate, 0.01f * range, 5f * range);
					EditorGUI.indentLevel -= 1;
				}

				if (m_spawner.m_spawnAlgorithm == Constants.LocationAlgorithm.Every)
				{
					seedThrowRange = EditorGUILayout.Slider(GetLabel("Throw Distance"), seedThrowRange, 0.5f, range);
				}
				else
				{
					seedThrowRange = EditorGUILayout.Slider(GetLabel("Throw Distance"), seedThrowRange, 0f, range);
				}
				...
				randomSeed = EditorGUILayout.IntField(GetLabel("Spawn Seed"), randomSeed);

				if (m_spawner.m_seedThrowRange > 0)
				{
			...
			else
			{
				...
				{
					EditorGUI.indentLevel += 1;
					flowRate = EditorGUILayout.FloatField(GetLabel("Flow Rate"), flowRate);
					EditorGUI.indentLevel -= 1;
				}
			...
			bool doSectorise = m_spawner.m_doSectorise;
			SECTR_Constants.ReparentingMode sectorReparentingMode = m_spawner.m_sectorReparentingMode;
			doSectorise = EditorGUILayout.BeginToggleGroup(ENABLE_SECTORISATION_LABEL, doSectorise);
			{
				sectorReparentingMode = (SECTR_Constants.ReparentingMode)EditorGUILayout.EnumPopup(GetLabel("Localize Ojbects by"), sectorReparentingMode);
			}
				...
				if (hasPrefabs == true || hasTrees == true || (m_spawner.m_critCheckMask && m_spawner.m_critMaskType == Constants.MaskType.Image))
				{
					...
					switch (rotationAlgorithm)
					{
						...
						case Constants.RotationAlgorithm.Fixed:
							EditorGUI.indentLevel++;
							minRotationY = EditorGUILayout.Slider(GetLabel("Fixed Rotation"), minRotationY, 0f, 360f);
			...
			LayerMask critSpawnCollisionLayers = m_spawner.m_critSpawnCollisionLayers;
			...
			bool showCritMinSpawnHeight = m_spawner.m_showCritMinSpawnHeight;
			float critMaxSpawnHeight = m_spawner.m_critMaxSpawnHeight;
			bool showCritMaxSpawnHeight = m_spawner.m_showCritMaxSpawnHeight;
			...
			if (showSpawnCriteria)
			{
			    ...
			    if (critVirginCheckType != Constants.VirginCheckType.None)
			    {
			        ...
			        critSpawnCollisionLayers = LayerMaskField(GetLabel("Collision Layers"), critSpawnCollisionLayers);
                }
				...
				if (critCheckHeight)
				{
					...
					if (advUseLargeRanges)
					{
						...
						EditorGUI.indentLevel += 1;
						showCritMinSpawnHeight = EditorGUILayout.Toggle(GetLabel("Show Min Height"), showCritMinSpawnHeight);
						EditorGUI.indentLevel -= 1;

						critMaxSpawnHeight = EditorGUILayout.FloatField(GetLabel("Max Height"), critMaxSpawnHeight);
						EditorGUI.indentLevel += 1;
						showCritMaxSpawnHeight = EditorGUILayout.Toggle(GetLabel("Show Max Height"), showCritMaxSpawnHeight);
						EditorGUI.indentLevel -= 1;
					...
					{
						critMinSpawnHeight = EditorGUILayout.Slider(GetLabel("Min Height"), critMinSpawnHeight, -20f, m_spawner.m_terrainTopBoundary);
						EditorGUI.indentLevel += 1;
						showCritMinSpawnHeight = EditorGUILayout.Toggle(GetLabel("Visualize"), showCritMinSpawnHeight);
						EditorGUI.indentLevel -= 1;

						critMaxSpawnHeight = EditorGUILayout.Slider(GetLabel("Max Height"), critMaxSpawnHeight, -20f, m_spawner.m_terrainTopBoundary);
						EditorGUI.indentLevel += 1;
						showCritMaxSpawnHeight = EditorGUILayout.Toggle(GetLabel("Visualize"), showCritMaxSpawnHeight);
						EditorGUI.indentLevel -= 1;
			...
			if (showPrototypes == true)
			{
				...
				for (int protoIdx = 0; protoIdx < protoList.Count; protoIdx++)
				{
					...
					if (proto.m_active == true)
					{
						...
						proto.m_displayedInEditor = EditorGUILayout.Foldout(proto.m_displayedInEditor, GetLabel("Detail"), true);
		    ...
		    float advSpawnCheckOffset = m_spawner.m_advSpawnCheckOffset;
            bool spawnToAVS = m_spawner.m_spawnToAVS;

			if (showAdvancedSettings)
			{
#if VEGETATION_STUDIO
                spawnToAVS = EditorGUILayout.Toggle(GetLabel("Spawn To AVS"), spawnToAVS);
#endif
			    ...
			    advSpawnCheckOffset = EditorGUILayout.FloatField(GetLabel("Collision Test Offset"), advSpawnCheckOffset);
				...
				if (advAutoOptimise)
				{
					advMaxSizeToOptimise = EditorGUILayout.Slider(GetLabel("  Smaller Than (m)"), advMaxSizeToOptimise, 5f, Constants.MaximumOptimisationSize);
				}
            ...
            */
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				m_spawner.m_spawnMode = spawnMode;

				if (flowRate != flowRateChecksum)
				{
					m_spawner.m_flowDistanceSqr = flowRate * flowRate;
				}
				...
				if (m_spawner.m_seedThrowRange != seedThrowRange)
				{
					if (seedThrowRange == 0)
					{
				...
				if (m_spawner.m_maxSpawnRange != range)
				{
					flowRate = Mathf.Clamp(flowRate, 0.01f * range, 5f * range);
					m_spawner.m_flowDistanceSqr = flowRate * flowRate;
...
#if SECTR_CORE_PRESENT
				m_spawner.m_doSectorise = doSectorise;
				m_spawner.m_sectorReparentingMode = sectorReparentingMode;
#endif

				m_spawner.m_showSpawnCriteria = showSpawnCriteria;
				if (m_spawner.m_spawnAlgorithm != spawnAlgorithm)
				{
					if (spawnAlgorithm == Constants.LocationAlgorithm.Every && seedThrowRange < 0.5f)
					{
						m_spawner.m_seedThrowRange = 0.5f;
					}
				...
				{
					case Constants.RotationAlgorithm.Fixed:
						if (m_spawner.m_rotationAlgorithm != rotationAlgorithm || m_spawner.m_minRotationY != minRotationY)
						{
							maxRotationY = minRotationY;
					...
					case Constants.RotationAlgorithm.LastSpawnClosest:
						if (m_spawner.m_rotationAlgorithm != rotationAlgorithm)
						{
							minRotationY = maxRotationY = 0f;
							m_spawner.m_rotationAlgorithm = rotationAlgorithm;
						}
						break;
					default:
				...
				m_spawner.m_critSpawnCollisionLayers = critSpawnCollisionLayers;
				m_spawner.m_critBoundsBorder = critBoundsBorder;
				m_spawner.m_critCheckHeight = critCheckHeight;
				if (m_spawner.m_critMinSpawnHeight != critMinSpawnHeight)
				{
					if (critMaxSpawnHeight < critMinSpawnHeight)
					{
						critMaxSpawnHeight = critMinSpawnHeight;
					}
					m_spawner.m_critMinSpawnHeight = critMinSpawnHeight;
				}
				m_spawner.m_showCritMinSpawnHeight = showCritMinSpawnHeight;
				if (m_spawner.m_critMaxSpawnHeight != critMaxSpawnHeight)
				{
					if (critMaxSpawnHeight < critMinSpawnHeight)
					{
						critMinSpawnHeight = critMaxSpawnHeight;
					}
					m_spawner.m_critMaxSpawnHeight = critMaxSpawnHeight;
				}
				m_spawner.m_critMaxSpawnHeight = critMaxSpawnHeight;
				m_spawner.m_showCritMaxSpawnHeight = showCritMaxSpawnHeight;
				...
				if (m_spawner.m_spawnPrototypes.Count == 0 && protoList.Count > 0)
				{
					...
					flowRate = Mathf.Round(100f * Mathf.Max(protoList[0].m_size.x, protoList[0].m_size.z)) * 0.01f + 0.05f;
					m_spawner.m_flowDistanceSqr = flowRate * flowRate;
				}
				...
				m_spawner.m_spawnToAVS = spawnToAVS;
			    ...
			    m_spawner.m_advSpawnCheckOffset = advSpawnCheckOffset;
		...
		void OnSceneGUI()
		{
			...
			if (Event.current.shift == true && Event.current.isMouse == true)
			{
				...
				if (Event.current.button == 0)
				{
					...
					else if (Event.current.type == EventType.MouseUp && GUIUtility.hotControl == m_editor_control_id)
					{
						GUIUtility.hotControl = 0;
					}
				}
				...
				if (Event.current.button == 0)
				{
					switch (Event.current.type)
					{
						case EventType.MouseDown:
							...
							if (Physics.Raycast(ray, out hitInfo, 10000f, m_spawner.m_critSpawnCollisionLayers))
							{
							    m_spawnLocations.Add(new SpawnCall(hitInfo.transform, hitInfo.point, hitInfo.normal));
							...
							if (GUIUtility.hotControl == m_editor_control_id && m_spawner.m_spawnMode == Constants.SpawnMode.Paint)
							{
								...
								if (Physics.Raycast(ray, out hitInfo, 10000f, m_spawner.m_critSpawnCollisionLayers))
								{
								    if (m_spawnLocations.Count > 0)
								    {
								        ...
								        {
								            m_spawnLocations.Add(new SpawnCall(hitInfo.point));
								            EditorApplication.delayCall += SpawnAfterEditorUpdate;
								        }
                                    }
                                    else
			...
			if (m_spawner.m_rotationAlgorithm <= Constants.RotationAlgorithm.Fixed)
			{
				if (HasTrees(m_spawner.m_spawnPrototypes) || HasPrefabs(m_spawner.m_spawnPrototypes) || m_spawner.m_critMaskType == Constants.MaskType.Image)
				{
                    ...
                    if (Spawner.ApproximatelyEqual(m_spawner.m_minRotationY, m_spawner.m_maxRotationY))
					{
                        ...
                        Handles.ArrowHandleCap(m_editor_control_id, m_spawner.m_spawnOriginLocation, visRot, Mathf.Clamp(m_spawner.m_maxSpawnRange / 6f, 0.25f, 1f), EventType.Repaint);
                    }
					else
					{
	    ...
	    void SpawnAfterEditorUpdate()
	    {
	        if (m_spawner != null)
	        {
                while (m_spawnLocations.Count > 0)
	            {
	                ...
	                if (!sc.m_isSubspawn)
	                {
	                    m_spawner.SetSpawnOriginAndUpdateRanges(sc.m_transform, sc.m_location, sc.m_normal);
                    }
		...
		void EditResource(Prototype proto, Resource res, bool indented)
		{
			...
			{
				//AVS
				if (m_spawner.m_spawnToAVS)
				{
					res.m_avsVegetationType = (Constants.AVSVegetationType)EditorGUILayout.EnumPopup(GetLabel("AVS Type"), res.m_avsVegetationType);
				}
					...
					foreach (UnityEngine.Object dragged_object in DragAndDrop.objectReferences)
					{
						if (dragged_object is GameObject)
						{
							...
							else
							{
								...
								if (pt != PrefabAssetType.NotAPrefab &&(pt == PrefabAssetType.Regular || pt == PrefabAssetType.Model))
#else
...
#if UNITY_2018_3_OR_NEWER
                if (PrefabUtility.GetPrefabAssetType(go) != PrefabAssetType.NotAPrefab && PrefabUtility.GetPrefabAssetType(go) != PrefabAssetType.MissingAsset)
#else
                if (PrefabUtility.GetPrefabType(go) == PrefabType.PrefabInstance)
#endif
                {
#if UNITY_2018_3_OR_NEWER
                    res.m_prefab = PrefabUtility.GetCorrespondingObjectFromOriginalSource(go) as GameObject;
		...
		{
			{ "Spawn Mode", "Set the required spawn mode.\n\nSingle: Each clicks result in a single spawn.\n\nPaint: Dragging the mouse will initiate additional spawns similar to painting."},
			{ "Flow Rate", "Distance at which spawns are initiated in Paint Mode."},
			...
			//SECTR integration
			{ "Localize Ojbects by", "Choose the method by which the parent sector of objects is determined. If going by bounds, " +
			...
			{ "Fit Within Mask", "Will ensure that the prototype is spawned only if it fits completely on top of the underlying mask. If not set then the object will be spawned on the mask, but its edges can also extend off it. If you use this then you need to make sure that the size of the masked area is big enough to fit the entire prototype."},
			{ "Check Height", "Check height when spawning."},
			{ "Min Height", "Minimum height that can be spawned."},
			{ "Show Min Height", "Visualised as blue plane as it can be used to represent water."},
			{ "Max Height", "Maximum height that can be spawned."},
			{ "Show Max Height", "Visualised as white(grey) plane as it can be used to represent snow and ice on top of a mountain."},
			...
			{ "Min Rotation", "Minimum Y rotation at what an object will be placed into a scene."},
			{ "Max Rotation", "Maximim Y rotation at what an object will be placed into a scene."},
			{ "Fixed Rotation", "Y rotation at what an object will be placed into a scene."},
			{ "Draggable Rotation", "When selected the rotation for a component can be dragged to update it to a given direction for precise rotation control. Only available for Fixed rotation."},
			{ "Fwd Rotation Offset", "This is added to the Y rotation. Mostly used with draggable rotation to correct for 'forward'."},
			{ "Min Scale", "Minimum placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Max Scale", "Maximum placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Min Scale X", "Minimum X placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Max Scale X", "Maximum X placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Min Scale Y", "Minimum Y placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Max Scale Y", "Maximum Y placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Min Scale Z", "Minimum Z placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
			{ "Max Scale Z", "Maximum Z placement scale. Final scale will be Placement Criteria scale * Prototype Scale."},
		    ...
		    { "Collision Test Offset", "When testing for collisions, this offset will be added to what you click on, to test for collisions above this location as well."},
		...
		/// Labels
		/// </summary>
		private readonly GUIContent FIRST_STEP_HELP_LABEL = new GUIContent("Add your Assets then...",
			"You can add your assets at the bottom via drag and drop, or the buttons (to add terrain trees/grass).");

		private readonly GUIContent VISUALISE_HELP_LABEL = new GUIContent("Visualise: Shift + Left click.",
			"This will visualise of your spawning according to your Placement and Spawning Criterias.");

		private readonly GUIContent ROTATION_HELP_LABEL = new GUIContent(
			"Drag Rotation: Shift + Left drag (requires Draggable Rotation).",
			"You can set Dragable Rotation under Placement Criteria -> Rotation Type: Fixed");

		private readonly GUIContent RANGE_HELP_LABEL = new GUIContent("Modify Range: Shift + Scroll wheel.",
			"Shortcut to adjust your Spawn Range (found under Overview)");

		private readonly GUIContent INSTANCES_HELP_LABEL = new GUIContent("Modify Instances: Ctrl + Scroll wheel.",
			"Shortcut to adjust Min Instances (found under Overview)");

		private readonly GUIContent MOVE_LAST_HELP_LABEL = new GUIContent("Move Horizontally Last Spawn: Ctrl + Arrows.",
			"Move horizontally the last spawned object. Useful for fine tuning.");

		private readonly GUIContent HEIGHT_AND_ROT_LAST_HELP_LABEL = new GUIContent("Raise-Lower / Rotate Last Spawn: Ctrl + Shift + Arrows.",
			"Move up/down and rotate the last spawned object. Useful for fine tuning.");

		private readonly GUIContent DELETE_ALL_HELP_LABEL = new GUIContent("Delete Instances: Ctrl + Backspace.",
			"Delete all instances that has been spawned by this Spawner.");

		private readonly GUIContent SINGLE_SPAWN_HELP_LABEL = new GUIContent("Single/Paint Spawn: Ctrl + Left click/drag. (See Spawn Mode)",
			"Single or paint spawn. Switch between modes in Overview.");

		private readonly GUIContent GLOBAL_SPAWN_HELP_LABEL = new GUIContent("Global Spawn: Ctrl + Shift + Left click.",
			"Spawn globally according to the settings.");

#if SECTR_CORE_PRESENT
		private readonly GUIContent ENABLE_SECTORISATION_LABEL = new GUIContent("SECTR Sectorisation",
				"Spawn objects in sectors.");
#endif
		...
		private void UpdateResourceIDs()
		{
			...
			string path;
			Terrain terrain = Terrain.activeTerrain;
			foreach (var proto in m_spawner.m_spawnPrototypes)
			{
				foreach (var resource in proto.m_resources)
				{
					if (string.IsNullOrEmpty(resource.m_assetID))
					{
